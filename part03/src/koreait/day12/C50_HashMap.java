...
public class C50_HashMap {
	...
	public static void main(String[] args) {
		...
		//데이터 접근(읽기) : get() 메소드
		System.out.println("2. 특정 key값의 value 출력");
		...
		// key 값 중복되면?
		menus.put("샐러드" , 8200);		// 실행결과? 덮어쓰기 됨
		System.out.println(menus);
...
//		menus.remove("샐러드" , 6000);		// 인자가 key. value : 키와 벨류가 모두 일치해야만 삭제가 됨
		menus.remove("샐러드" , 8200);		// 인자가 key. value : 키와 벨류가 모두 일치해야만 삭제가 됨
		System.out.println("삭제 결과 확인 : " + menus);
		}
...
/*	ArrayList : 순차적(인덱스로 접근) 으로 데이터를 관리하는 자료구조입니다.배열과 유사한 구조입니다.
	HashMap : 순차적인 구조가 아닙니다. 인덱스를 사용하지 앖고 Key 값을 이용하여 데이터접근하여 저장/읽기 합니다.
			...
			*key와 velue 는 모든 타입이 가능하다. 
			* 	(hashmap 에서는 key, value 한쌍으로 요소가 된다. value 검색하기 위해서 key를 사용한다.)
