...
public class AnimalTest4 {
	...
	public static void main(String[] args) {
		...
		Dog puddle = new Dog();
		Crow crow = new Crow();
		// 자식 객체가 생성될때 먼저 부모클래스 객체를 생성하면서 부모클래스 생성자를 실행
		Dog dog = new Dog();
	...
	// 다형성
	// 구현 예시 1 : 메소드 인자(또는 리턴)  타입이 Animal  : Animal, Dog, Crow, Cat, Frog 객체 모두 가능
	public static void animal (Animal animal) {
		// 구현 예시 2 : 메소드 재정의. animal 객체 인스턴스 타입에 따라서 실행되는 act 결과가 다르다.
		animal.act();
		
		
		animal.print();
		// 인자로 전달 받은 animal 객체가 참조하는 것의 진짜 타입 (인스턴스 타입) 검사 : instanceof
		System.out.println("dog? " + (animal instanceof Dog));		// 객체 참조 변수 instanceof  클래스 명
		...
		/*		
		 * 	Animal 부모타입 참조는 자식클래스 필드와 메소드 접근 못 하지롱
		animal.jump();		// frog
		animal.run();		// dog
		animal.fly();		// crow
		animal.sound();		// cat
		*/	
		...
		if (animal instanceof Cat) {
			cat = (Cat)animal;		// 다운캐스팅 : animal 진짜 객체에 따라 오류가 난다 또는 오류 없음.
			cat.sound();
		}
...
//				super.act();		// 부모 클래스의 act() 실행
		...
		class Cat extends Animal{
			...
			@Override
			public void act() {
				System.out.println("Animal 행동 : 수시로 사고치고 다니기");
			}
		}