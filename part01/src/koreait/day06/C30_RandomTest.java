...
public class C30_RandomTest {
	...
	public static void main(String[] args) {
		Random r = new Random();		// 메소드를 사용하기 위한 Random 객체 생성(new 연산)
		
		
		System.out.println("1. 정수형 랜덤 값 10개 출력");
		for (int i = 0; i < 10; i++) {
			int inum = r.nextInt(); // int 범위내에서 랜덤 값(난수) 만든다.
		...
		// 1 <= 난수 < 101 범위의 랜덤 값을 만들어야 한다면?
		System.out.println("3. 특정 범위의 랜덤 값 10개 출력 = bound를 이용한 수식");
		for (int i = 0; i < 10; i++) {
			int inum = r.nextInt(100)+1; // 0 <= 난수 < 101 범위의 랜덤값 만든다.
			System.out.println(inum);
		}
