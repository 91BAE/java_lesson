...
// char 배열의 특징 및 문자열과 연관된 내용 파악하기!
	...
	public static void main(String[] args) {
		...
		char [] carr2 = new char[20];	// 초기값은 지정 안하면 0이다
		System.out.println("[[[[ char 배열 테스트 - 1 ]]]]");
		System.out.println("carr1 : " + Arrays.toString(carr1));
		System.out.println("carr2 : " + Arrays.toString(carr2));	// 0에 대한 문자는 빈칸으로 나온다.
		System.out.println("carr1 배열의 길이 (크기) : " + carr1.length);
		System.out.println("carr2 배열의 길이 (크기) : " + carr2.length);
		
		System.out.println("carr1 배열 3번째 데이터 값 (문자) : " + carr1[2]);	// 값?
		System.out.println("carr2 배열 3번째 데이터 값 (문자) : " + carr2[2]);	// 값?
		
		
		System.out.println("carr1 배열 3번째 데이터 값 (정수) : " + (int)carr1[2]);	// 값? 109
		System.out.println("carr2 배열 3번째 데이터 값 (정수) : " + (int)carr2[2]);	// 값? 0
		...
		System.out.println("\3. 배열이름(참조형 변수)으로 출력");
		System.out.println(carr1);	// 위의 2번 for문과 동일한 결과 - 정수형 배열과 다르게 문자 열 참조변수처럼 동작한다.
		...
		// = hello hi goodbye ; // char 배열에 바로 문자열 대입은 오류
		...
		carr2 = "hello hi goodbye hi goodbye".toCharArray(); // 문자열이 선언된 배열 크기보다 큼
		System.out.println(carr2);
		System.out.println("carr2.length : " + carr2.length );		// 27
		
		System.out.println("\n5. 배열을 문자열로 변경");
		String temp = String.valueOf(carr2);		// char[] 배열을 문자열로 변환
													// String 객체의 toCharArray() 메소드와 반대
		// 참고 : String.valueOf 메소드는 여러가지 매개 변수 형식을 다룬다. (메소드 overloading)
