...
public class C34_StringArray {
	...
	public static void main(String[] args) {
		...
		// int, double, char 배열에서는 요소를 나타내는 nums[2], carr1[2], double[2], point[2]
		//										  ㄴ 기본형 데이터 타입
		...
		String[] koreans = new String[5];	// 배열 요소의 값이 기본형 데이터 저장x, 참조값 저장
											// 기본 초기값은? null (아무것도 참조하지 않았기 때문에)
		...
		for(int i = 0; i<koreans.length; i++) 
			if(koreans[i] != null)	// ※※ 아주 중요한 if 문 예시
...
//		System.out.println(temp.length());	// 오류
		...
		for (i=0; i<engkey.length;i++) {
			if(find.equals(engkey[i]))
				break;		// i=0 ~ engkey.lengt-1
		}
		// break 로 for를 모두 실행했을때 같은 문자열이 배열 engkey에 없을 때 입니다.
		...
		for (int i=0; i<engkey.length; i++) {
			if (find.equals(engkey[i])) {		// 일치하는 단어 찾으면 for 문 종료하기
				index = i;
				break;
			}
		}
