...
//C35 클래스를 1. 객체로 만들어서 2. Static 인 것들을 테스트 한다.
	// 참고 : main 메소드를 갖는 클래스를 객체로 만들지 않는 것이 일반적이다.
	public static void main(String[] args) {
		...
		// C35 클래스에서 Static 이 아닌 인스턴스 변수에는 객체마다 다른 값을 저장합니다.	- 객체 각각 다른 특성을 나타낸다.
		test1.count = 10;
		test2.count = 20;
		
		test1.message="hi test1";
		test2.message="hello test1";
		
		test1.methodA();		// 인스턴스 메소드
		test2.methodA();		
		
		test1.methodC(123);		// this.count 에서 this는 test1
		test2.methodC(999);		// this.count 에서 this는 test2
...
//		C35_GlobalVar.TEST = 100;		// 오류 : final 은 값 변경 못합니다.
		...
		C35_GlobalVar test3 = new C35_GlobalVar();
			// 전역 변수의 기본 초기값? - 0, 0.0 , 참조타입은 null
		...
		System.out.println("test3 count = " + C35_GlobalVar.point);	// 0.0
	}
